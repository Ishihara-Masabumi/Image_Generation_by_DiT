...
def main():
    ...
    optimizer = optim.Adam(instaf.parameters(), lr=1e-5)
    epochs = 100
    ...
    for epoch in range(epochs):
        total_loss = 0
        # triplets全体を回して学習
        for labels, _, images in tqdm(triplets, desc=f"Epoch {epoch+1} (2-Rectified)"):
            images = images.to(device)
            labels = labels.to(device)

            # 毎バッチでペアを生成
            z0, z1 = instaf.generate_pairs(images, labels)

            # reflow学習
            loss, _ = instaf.reflow(z0, z1, labels)
